...
wb_sheet_final = load_workbook('12.21汇总_表格_final.xlsx')
# 获取所有工作表的名称
sheet_names = wb_sheet_final.sheetnames
print(sheet_names)
esp_pic_name=sheet_names[11]+'_'
...
# norm = cm.colors.TwoSlopeNorm(vmin=minium_num, vcenter=0, vmax=maxium_num)
...
#     cbar = fig.colorbar(im,ticks=[minium_num,0,maxium_num])
...
#         if Z_matrix[j, i] <= threshold_1 + 0.02 and Z_matrix[j, i] >= threshold_1 - 0.02:
#             x_curve.append(i + 1)
#             y_curve.append(j + 5)
...
# for i in range(Z_matrix.shape[1]):
#     for j in range(Z_matrix.shape[0]):
#         if Z_matrix[j, i] <= threshold_3 + 0.02 and Z_matrix[j, i] >= threshold_3 - 0.02:
#             x_curve.append(i + 1)
#             y_curve.append(j + 5)
#             break  # 只选取每列的第一个满足条件的点
...
# bSavePicAndShow = False
# if bSavePicAndShow:
#     plt.savefig(esp_pic_name+'.eps', format='eps',dpi=300)
#     plt.savefig(esp_pic_name+'.png', dpi=300)
#     plt.show()
